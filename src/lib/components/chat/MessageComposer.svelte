...
<div class="border-t border-zinc-200 bg-white px-3 pt-2 pb-3 dark:border-zinc-800 dark:bg-zinc-900">
	{#if editing}
		...
		>
			...
			<button
				...
				class="ml-2 rounded text-zinc-400 hover:text-zinc-600 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none dark:hover:text-zinc-200"
				>✕</button
			>
		</div>
	{:else if replyTo}
			...
			{#each attachments as file, i (file)}
				...
				>
					...
					<button
						...
						class="rounded text-zinc-400 hover:text-red-500 focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none"
						>✕</button
					>
				</div>
			{/each}
	...
	{#if recording}
		...
		>
			...
			<button
				...
				class="ml-auto font-medium hover:underline focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none"
				>Cancel</button
			>
			<button
				onclick={() => stopRecording(true)}
				aria-label="Send voice note"
				class="rounded-md bg-red-600 px-3 py-1 font-medium text-white hover:bg-red-500 focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:outline-none"
				>Send</button
			>
		</div>
	{/if}
	...
	<div class="flex items-end gap-1.5">
		...
		<label
			class="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full text-zinc-500 transition focus-within:ring-2 focus-within:ring-indigo-500 focus-within:outline-none hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-800"
			title="Attach file"
		>
			<svg
				class="h-5 w-5"
		...
		<textarea
			...
			class="max-h-[140px] min-h-[40px] flex-1 resize-none rounded-2xl border-zinc-200 bg-zinc-100 px-4 py-2.5 text-sm focus:border-indigo-400 focus:ring-indigo-400 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none dark:border-zinc-700 dark:bg-zinc-800"
		></textarea>
		...
		{:else if text.trim() || attachments.length > 0 || editing}
			<button
				...
				class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-white transition hover:bg-indigo-500 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none"
			>
				<svg
					class="h-5 w-5"
					viewBox="0 0 24 24"
