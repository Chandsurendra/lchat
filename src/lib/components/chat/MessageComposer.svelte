	...
	{#if editing}
		...
		>
			...
			<button
				...
				class="ml-2 rounded text-zinc-400 outline-none hover:text-zinc-600 focus-visible:ring-2 focus-visible:ring-indigo-500 dark:hover:text-zinc-200"
				>✕</button
			>
		</div>
	{:else if replyTo}
			...
			{#each attachments as file, i (file)}
				...
				>
					...
					<button
						...
						class="rounded text-zinc-400 outline-none hover:text-red-500 focus-visible:ring-2 focus-visible:ring-indigo-500"
						>✕</button
					>
				</div>
			{/each}
	...
	<div class="flex items-end gap-1.5">
		<div class="relative">
			<button
				...
				class="flex h-10 w-10 items-center justify-center rounded-full text-zinc-500 transition outline-none hover:bg-zinc-100 focus-visible:ring-2 focus-visible:ring-indigo-500 dark:text-zinc-400 dark:hover:bg-zinc-800"
				>😊</button
			>
			{#if showEmoji}
		...
		<label
			class="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full text-zinc-500 transition outline-none focus-within:ring-2 focus-within:ring-indigo-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-zinc-800"
			title="Attach files"
		>
			<svg
				class="h-5 w-5"
				...
				/></svg
			>
			<input type="file" multiple class="sr-only" aria-label="Attach files" onchange={onFiles} />
		</label>
		...
		{:else if text.trim() || attachments.length > 0 || editing}
			<button
				...
				class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-white transition outline-none hover:bg-indigo-500 focus-visible:ring-2 focus-visible:ring-indigo-500"
			>
				<svg
					class="h-5 w-5"
					viewBox="0 0 24 24"
		...
		{:else}
			<button
				...
				class={cn(
					'flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition outline-none focus-visible:ring-2 focus-visible:ring-indigo-500',
					recording
